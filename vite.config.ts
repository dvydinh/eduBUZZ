...
import siteConfiguration from './.app/make/site.json'

// Vite config — https://vitejs.dev/config/
export default defineConfig(({ mode }) => {
  // .app/make/deploy-preview passes `--mode development` for cached-preview builds.
  ...
  return {
    ...
    plugins: [
      react(),
      tailwindcss(),
      appSiteConfiguration(siteConfiguration),
      appErrorOverlayReplay(),
      appReactRefreshBoundaryFallback(),
      appMakeKitPlugin({ storiesGlob: '/src/**/*.stories.{ts,tsx,js,jsx}' }),
    ],
    resolve: {
      alias: {
...
/** Applies /.app/make/site.json to the generated document shell. */
function appSiteConfiguration(config: appSiteConfiguration): Plugin {
  ...
  const title = config.title ?? "app Make App"
 ...
 * fixed build.
 */
function appErrorOverlayReplay(): Plugin {
  return {
    name: 'app-error-overlay-replay',
    apply: 'serve',
    configureServer(server) {
 ...
 */
function appReactRefreshBoundaryFallback(): Plugin {
  const hadRefreshBoundary = new Map<string, boolean>()
  let sendFullReload: (() => void) | null = null

  return {
    name: 'app-react-refresh-boundary-fallback',
    apply: 'serve',
    enforce: 'post',
    configureServer(server) {
...
/**
 * Serves a blank render-target page at /.app/make/kit.html that
 * the app preview script drives directly. The page exposes a
 * registry of every file matching `storiesGlob` on
 * window.__app__.stories so the design surface can dynamically
 ...
 */
function appMakeKitPlugin(options: { storiesGlob: string | string[] }): Plugin {
  const storiesGlob = Array.isArray(options.storiesGlob) ? options.storiesGlob : [options.storiesGlob]
  const ROUTE = '/.app/make/kit.html'
  const VIRTUAL_ID = 'virtual:app-stories'
...
<body>
<div id="app-make-kit-root"></div>
<script type="module">
  import { stories } from 'virtual:app-stories'
  window.__app__ = Object.assign(window.__app__ ?? {}, { stories })
